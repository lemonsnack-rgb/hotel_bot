...
def extract_price(value: Any) -> Optional[int]:
    """
    SerpAPI Google Hotels 응답의 rate_per_night / total_rate 등은
    "$123", "₩150,000", {"lowest": "..."} 등 다양한 형태일 수 있으므로
    최대한 방어적으로 숫자를 추출한다.
    """
    if value is None:
        return None
    ...
    if isinstance(value, dict):
        for key in ("extracted_lowest", "lowest", "extracted_before_taxes_fees", "before_taxes_fees"):
            if key in value:
