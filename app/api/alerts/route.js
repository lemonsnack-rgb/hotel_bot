...
function validateAlert(alert) {
  if (!alert.email) return '이메일을 입력하세요.';
  if (!alert.location) return '도시 / 지역을 입력하세요.';
  if (!alert.check_in_date) return '체크인 날짜를 입력하세요.';
  if (!alert.check_out_date) return '체크아웃 날짜를 입력하세요.';
  if (alert.check_in_date >= alert.check_out_date) return '체크아웃 날짜는 체크인 날짜보다 뒤여야 합니다.';
  if (alert.adults < 1) return '성인 수는 1명 이상이어야 합니다.';
  if (alert.children < 0) return '어린이 수는 0명 이상이어야 합니다.';
  return null;
}
